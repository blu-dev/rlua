...
use std::{ops::DerefMut, ptr::addr_of_mut};
...
#[macro_export]
macro_rules! l_access {
    (top, $l:ident) => {{
        *(($l as *const u8).add(0x10) as *const *mut TValue)
    }};
    (ci, $l:ident) => {{
        *(($l as *const u8).add(0x20) as *const *mut u8)
    }};
    (ci_func, $l:ident) => {{
        let ci = l_access!(ci, $l);
        *(ci as *const *mut TValue)
    }};
}
...
macro_rules! inc_top {
    ($l:ident) => {{
        *(($l as *const u8).add(0x10) as *mut *mut TValue) =
            (*(($l as *const u8).add(0x10) as *mut *mut TValue)).add(1);
    }};
}
...
pub fn index2addr(L: *mut lua_State, idx: ::std::os::raw::c_int) -> *mut TValue {
    unsafe {
        let ci = l_access!(ci, L);
        if idx > 0 {
            let o = l_access!(ci_func, L).add(idx as usize);
            if o >= l_access!(top, L) {
                return addr_of_mut!(luaO_nilobject);
            } else {
                return o;
            }
        } else if !ispseudo!(idx) {
            return l_access!(top, L).offset(idx as isize);
        } else if idx == LUA_REGISTRYINDEX {
            return (*((L as *const u8).add(0x18) as *const *mut u8)).add(0x40) as *mut TValue;
        } else {
            let idx = LUA_REGISTRYINDEX - idx;
            if (*l_access!(ci_func, L)).tt == (LUA_TFUNCTION | (1 << 4)) {
                return addr_of_mut!(luaO_nilobject);
            } else {
                let closure = (*l_access!(ci_func, L)).value.object as *mut *mut u8;
                let upval_count = *closure.add(10);
                if idx <= (upval_count as _) {
                    (closure.add(0x10) as *mut TValue).add(idx as usize)
                } else {
                    return addr_of_mut!(luaO_nilobject);
                }
            }
...
pub fn lua_absindex(L: *mut lua_State, idx: ::std::os::raw::c_int) -> ::std::os::raw::c_int {
    unsafe {
        if idx > 0 || ispseudo!(idx) {
            ...
            (l_access!(top, L).offset_from(l_access!(ci_func, L)) + 1) as _
        }
    }
}

pub fn lua_gettop(L: *mut lua_State) -> ::std::os::raw::c_int {
    unsafe { l_access!(top, L).offset_from(l_access!(ci_func, L).add(1)) as _ }
}

pub fn lua_settop(L: *mut lua_State, idx: ::std::os::raw::c_int) {
    unsafe {
        let func = l_access!(ci_func, L);
        if idx >= 0 {
            while l_access!(top, L) < func.add((1 + idx) as usize) {
                (*l_access!(top, L)).tt = LUA_TNIL;
                *((L as *const u8).add(0x10) as *mut *mut TValue) = l_access!(top, L).add(1);
            }
        } else {
            *((L as *const u8).add(0x10) as *mut *mut TValue) =
                l_access!(top, L).offset((idx + 1) as isize);
        }
    }
}
...
pub fn lua_pushvalue(L: *mut lua_State, idx: ::std::os::raw::c_int) {
    unsafe {
        *l_access!(top, L) = *index2addr(L, idx);
        inc_top!(L);
    }
}

pub fn reverse(L: *mut lua_State, mut from: *mut TValue, mut to: *mut TValue) {
    unsafe {
        while from < to {
            let mut tmp = TValue {
                value: Value {
...
pub fn lua_rotate(L: *mut lua_State, idx: ::std::os::raw::c_int, n: ::std::os::raw::c_int) {
    unsafe {
        let t = l_access!(top, L).offset(-1);
        let p = index2addr(L, idx);
        let m = if n >= 0 {
            t.offset(-n as isize)
        } else {
            p.offset((-n - 1) as isize)
        };
        reverse(L, p, m);
        reverse(L, m.add(1), t);
        reverse(L, p, t);
...
#[skyline::from_offset(0x38f29d0 + 0xca0 + 0x3e0 + 0x5b0)]
...
#[skyline::from_offset(0x38f2b70 + 0xca0 + 0x3e0 + 0x5b0)]
...
#[skyline::from_offset(0x38f2e10 + 0xca0 + 0x3e0 + 0x5b0)]
...
#[skyline::from_offset(0x38f2f80 + 0xca0 + 0x3e0 + 0x5b0)]
...
pub fn lua_toboolean(L: *mut lua_State, idx: ::std::os::raw::c_int) -> ::std::os::raw::c_int {
    ...
    unsafe {
        match (*value).tt {
            LUA_TNIL => 0,
            LUA_TBOOLEAN => (*value).value.boolean,
            _ => 1,
        }
    }
}

#[skyline::from_offset(0x38f3100 + 0xca0 + 0x3e0 + 0x5b0)]
...
#[skyline::from_offset(0x38f2c90 + 0xca0 + 0x3e0 + 0x5b0)]
pub fn lua_compare(
    L: *mut lua_State,
    idx1: ::std::os::raw::c_int,
    idx2: ::std::os::raw::c_int,
...
#[skyline::from_offset(0x38f4450 + 0x5b0)]
...
#[skyline::from_offset(0x38f3710 + 0xca0 + 0x3e0 + 0x5b0)]
pub fn auxgetstr(
    L: *mut lua_State,
    value: *mut TValue,
...
pub fn lua_getfield(
    ...
    unsafe { auxgetstr(L, index2addr(L, idx), k) }
}
...
#[skyline::from_offset(0x391ca20 + 0xca0 + 0x3e0 + 0x5b0)]
pub fn luaH_new(L: *mut lua_State) -> *mut ::std::os::raw::c_void;

#[skyline::from_offset(0x391be90 + 0xca0 + 0x3e0 + 0x5b0)]
pub fn luaH_resize(
    L: *mut lua_State,
...
pub fn lua_getuservalue(L: *mut lua_State, idx: ::std::os::raw::c_int) -> ::std::os::raw::c_int {
    unimplemented!()
}
...
#[skyline::from_offset(0x38f3d60 + 0xca0 + 0x3e0 + 0x5b0)]
...
pub fn lua_setfield(
    ...
    unsafe { auxsetstr(L, index2addr(L, idx), k) }
}
...
#[skyline::from_offset(0x38f45d0 + 0xca0 + 0x3e0 + 0x5b0)]
...
pub fn lua_upvalueid(L: *mut lua_State, fidx: ::std::os::raw::c_int, n: ::std::os::raw::c_int) {
    unimplemented!()
}
...
pub extern "C" fn luaopen_os(L: *mut lua_State) -> ::std::os::raw::c_int {
    unimplemented!()
}

pub extern "C" fn luaopen_bit32(L: *mut lua_State) -> ::std::os::raw::c_int {
    unimplemented!()
}

pub extern "C" fn luaopen_debug(L: *mut lua_State) -> ::std::os::raw::c_int {
    unimplemented!()
}

pub fn luaL_openlibs(L: *mut lua_State) {
    unsafe {
        luaL_requiref(L, "_G\0".as_ptr() as _, Some(luaopen_base), 1);
        luaL_requiref(L, "package\0".as_ptr() as _, Some(luaopen_package), 1);
        luaL_requiref(L, "coroutine\0".as_ptr() as _, Some(luaopen_coroutine), 1);
        luaL_requiref(L, "table\0".as_ptr() as _, Some(luaopen_table), 1);
        luaL_requiref(L, "string\0".as_ptr() as _, Some(luaopen_string), 1);
        luaL_requiref(L, "math\0".as_ptr() as _, Some(luaopen_math), 1);
        luaL_requiref(L, "utf8\0".as_ptr() as _, Some(luaopen_utf8), 1);
    }
}
...
#[skyline::from_offset(0x38f5c30 + 0xca0 + 0x3e0 + 0x5b0)]
...
#[skyline::from_offset(0x38f7ee0 + 0xca0 + 0x3e0 + 0x5b0)]
...
#[skyline::from_offset(0x38f8600 + 0xca0 + 0x3e0 + 0x5b0)]
